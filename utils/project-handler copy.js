  ...
  projectHandler (projectList) {
    return projectList.map(item => {
      ...
      item.plans.forEach((plan, idx) => {
        if (plan.task_type === 'normal') {
          ...
          if (new Date() >= new Date(plan.end_time)) {
            item.project.expectant_progress_day += plan.task_time
            developers[plan.developer_name].expectant_progress_day += plan.task_time
          }
      ...
      item.developers = [...Object.values(developers)].map(dp => {
        dp.progress_day = parseFloat((dp.progress / dp.task_nums * dp.task_time).toFixed(2))
        dp.progress = parseFloat((dp.progress / dp.task_nums * 100).toFixed(2))        
        dp.expectant_progress_day = parseFloat((dp.expectant_progress / dp.task_nums * dp.task_time).toFixed(2))
        ...
        dp.delay = Math.ceil((dp.expectant_progress_day - dp.progress_day).toFixed(2))
        item.project.expectant_progress_day += dp.expectant_progress_day
        item.project.progress_day += dp.progress_day
        return dp
      })
      ...
      item.project.delay =  Math.ceil((item.project.expectant_progress_day - item.project.progress_day).toFixed(2))
